...
void print_results(float array[N], int tid, int section) {
    ...
    #pragma omp critical
    {
        printf("Thread %d: resultados da seção %d\n", tid, section);
        for (i = 0; i < N; i++) {
            printf("%e  ", array[i]);
            j++;
            if (j == 6) {
...
int main (int argc, char *argv[]) {
    ...
    #pragma omp parallel num_threads(NTHREADS) private(c,i,tid,section)
    {
        ...
        #pragma omp barrier /* só continua depois da mensagem do número de threads */
        printf("Thread %d: a começar\n", tid);
        #pragma omp barrier /* só continua depois de todos imprimirem as mensagens iniciais */
